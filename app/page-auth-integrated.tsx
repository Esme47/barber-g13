...
import { getAuthContext, signOut as signOutAuth, type AuthContext } from "@/lib/auth";
...
type Client = { id:string; name:string; phone:string; email?:string; visits:number; lastService:string; lastVisit?:string|null; nextAppointment?:string|null; registeredAt:string; createdAt:string; };
...
const uiToDbStatus = (s: AppointmentStatus): DbAppointmentStatus => {
  const map: Record<AppointmentStatus, DbAppointmentStatus> = {
    "Pendiente": "pending",
...
export default function Home() {
  ...
  const isBarber=authContext?.role==="barber";
  const staffName=authContext?.staffProfile.full_name||"";
  const barberId=authContext?.barberId||null;
  const profileInitials=(staffName||"Usuario").split(" ").filter(Boolean).slice(0,2).map(w=>w[0]).join("").toUpperCase();
  const roleLabel=isAdmin?"ADMINISTRADOR":"BARBER";
  const menu=isAdmin
    ? [{name:"Dashboard",icon:<Icon.Grid/>},{name:"Agenda",icon:<Icon.Calendar/>},{name:"Clientes",icon:<Icon.Users/>},{name:"Servicios",icon:<Icon.Scissors/>},{name:"Finanzas",icon:<Icon.Wallet/>}]
    : [{name:"Dashboard",icon:<Icon.Grid/>},{name:"Agenda",icon:<Icon.Calendar/>},{name:"Clientes",icon:<Icon.Users/>}];
  ...
  useEffect(()=>{
    ...
    const loadAuth=async()=>{
      ...
      if(context?.role==="barber"){
        setActive(current=>["Dashboard","Agenda","Clientes"].includes(current)?current:"Dashboard");
      }
  ...
  const loadData = useCallback(async()=>{
    ...
    try {
      ...
      const [clientsRes,servicesRes,barbersRes,appointmentsRes]=await Promise.all([
        supabase.from("customer_profiles").select("id,full_name,phone,email,created_at,visits,last_service,last_visit,next_appointment").order("created_at",{ascending:false}),
      ...
      setClients((clientsRes.data||[]).map((c:any)=>({
        ...
        lastVisit:c.last_visit||null, nextAppointment:c.next_appointment||null,
        registeredAt:formatRegisteredDate(c.created_at), createdAt:c.created_at
      })));
      ...
      if(isAdmin){
        const transactionsRes=await supabase.from("transactions").select("*").order("created_at",{ascending:false});
  ...
  },[selectedDate,authContext,isAdmin]);

  useEffect(()=>{ if(authContext)loadData(); },[loadData,authContext]);

  useEffect(()=>{
    if(authContext && !menu.some(item=>item.name===active))setActive("Dashboard");
  },[authContext,active,menu.length]);
  ...
  const openAppointmentModal=(time?:string,client?:Client)=>{
    ...
    const firstBarber=isBarber&&barberId?barbers.find(b=>b.id===barberId):barbers[0];
    setAppointmentForm({
      name:client?.name||"", phone:client?.phone||"", serviceId:firstService?.id||"",
      barberId:firstBarber?.id||barberId||"", time:time||"09:00", notes:""
    });
  ...
  const handleCreateAppointment=async(e:React.FormEvent)=>{
    e.preventDefault();
    const effectiveBarberId=isBarber?barberId:appointmentForm.barberId;
    if(!appointmentForm.name.trim()||!appointmentForm.phone.trim()||!appointmentForm.serviceId||!effectiveBarberId) {
    ...
    try {
      ...
      const {error:appointmentError}=await supabase.from("appointments").insert({
        customer_id:customerId, barber_id:effectiveBarberId, service_id:appointmentForm.serviceId,
  ...
  const handleCreateClient=async(e:React.FormEvent)=>{
    ...
    try {
      ...
      const {data:existing,error:existingError}=await supabase.from("customers").select("id").eq("phone",normalizedPhone).maybeSingle();
      if(existingError) throw existingError;
      if(existing){ alert("Ya existe un cliente registrado con este teléfono."); return; }
      const {error}=await supabase.from("customers").insert({full_name:clientForm.name.trim(),phone:normalizedPhone,email:clientForm.email.trim()||null});
      if(error) throw error;
      setClientForm({name:"",phone:"",email:""}); setShowClientModal(false); await loadData();
    } catch(err:any){ console.error("Error guardando cliente:",err); alert("Error guardando cliente: "+(err?.message||"Error desconocido")); } finally {setSaving(false);}
  };
  ...
  const renderDashboard=()=> (
    <>
      <section className="welcome"><div><p className="section-label">PANEL DE CONTROL</p><h1>Buenos días, <em>{staffName||"G13"}.</em> 👋</h1><p className="welcome-text">Aquí tienes el resumen de tu barbería.</p></div><button className="primary-button" onClick={()=>{setActive("Agenda");openAppointmentModal()}}><Icon.Plus/>Nueva reserva</button></section>
      <section className="stats-grid">
        ...
        {isAdmin&&<article className="stat-card"><div className="stat-header"><span>INGRESOS</span><Icon.Wallet/></div><h2>{formatCurrency(financialSummary.sales)}</h2><p><strong>Ingresos registrados</strong></p></article>}
  ...
  const renderAgenda=()=> (
    <section className="agenda-page"><div className="agenda-heading"><div><p className="section-label">GESTIÓN DE RESERVAS</p><h1>Agenda <em>Profesional</em></h1><p className="welcome-text">Administra las citas y disponibilidad de {staffName||"BARBER G13"}.</p></div><button className="primary-button" onClick={()=>openAppointmentModal()}><Icon.Plus/>Nueva reserva</button></div>
